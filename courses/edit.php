<?php
require_once __DIR__ . '/../db.php';
$pdo = get_pdo();
$pageTitle = '編輯課程資料';
$bodyClass = 'form-page';

$courseId = $_GET['id'] ?? '';
if ($courseId === '') {
    header('Location: list.php?err=' . urlencode('缺少課程代號'));
    exit;
}

$stmt = $pdo->prepare('SELECT 課程代號, 課程名稱, 學分數, 老師編號 FROM Course WHERE 課程代號 = :id');
$stmt->execute([':id' => $courseId]);
$course = $stmt->fetch();

if (!$course) {
    header('Location: list.php?err=' . urlencode('找不到課程資料'));
    exit;
}

$teacherStmt = $pdo->query('SELECT 老師編號, 老師姓名 FROM Teacher ORDER BY 老師編號');
$teachers = $teacherStmt->fetchAll();

$errors = [];
$courseName = $course['課程名稱'];
$credits = $course['學分數'];
$teacherId = $course['老師編號'];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $courseName = trim($_POST['course_name'] ?? '');
    $credits = trim($_POST['credits'] ?? '');
    $teacherId = trim($_POST['teacher'] ?? '');

    if ($courseName === '' || $credits === '' || $teacherId === '') {
        $errors[] = '請填寫完整資料。';
    }

    if (!$errors) {
        $update = $pdo->prepare('UPDATE Course SET 課程名稱 = :name, 學分數 = :credits, 老師編號 = :teacher WHERE 課程代號 = :id');
        $update->execute([
            ':name' => $courseName,
            ':credits' => (int)$credits,
            ':teacher' => $teacherId,
            ':id' => $courseId,
        ]);
        header('Location: list.php?msg=' . urlencode('更新課程成功'));
        exit;
    }
}

include __DIR__ . '/../header.php';
?>

<?php if ($errors): ?>
    <div class="alert error"><?php echo h(implode(' ', $errors)); ?></div>
<?php endif; ?>

<div class="form-card">
    <div class="form-title">編輯課程資料</div>
    <form method="post" class="form-grid">
        <div class="form-row inline">
            <label>課程代號：</label>
            <input type="text" value="<?php echo h($courseId); ?>" readonly>
        </div>
        <div class="form-row inline">
            <label for="course_name">課程名稱：</label>
            <input type="text" id="course_name" name="course_name" value="<?php echo h($courseName); ?>" required>
        </div>
        <div class="form-row inline">
            <label for="credits">學分數：</label>
            <input type="number" id="credits" name="credits" value="<?php echo h($credits); ?>" min="1" required>
        </div>
        <div class="form-row inline">
            <label for="teacher">授課老師：</label>
            <select id="teacher" name="teacher" required>
                <?php foreach ($teachers as $teacher): ?>
                    <option value="<?php echo h($teacher['老師編號']); ?>" <?php echo $teacherId === $teacher['老師編號'] ? 'selected' : ''; ?>>
                        <?php echo h($teacher['老師姓名']); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="form-actions">
            <button class="btn primary" type="submit">更新課程</button>
            <a class="btn secondary" href="list.php">返回課程列表</a>
        </div>
    </form>
</div>

<?php include __DIR__ . '/../footer.php'; ?>
